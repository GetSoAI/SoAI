...
def checkTikaServer(scheme="http", serverHost=ServerHost, port=Port, tikaServerJar=TikaServerJar, classpath=None, config_path=None):
    ...
    if 'localhost' in serverEndpoint or '127.0.0.1' in serverEndpoint:
        if urlp.scheme != '':
            raise RuntimeError('A local Tika server jar path is required.')
        jarFilename = os.path.basename(tikaServerJar)
        versionPrefix = 'tika-server-standard-'
        versionSuffix = '.jar'
        if not jarFilename.startswith(versionPrefix) or not jarFilename.endswith(versionSuffix):
            raise RuntimeError('The local Tika server jar must use its standard versioned filename.')
        expectedVersion = jarFilename[len(versionPrefix):-len(versionSuffix)]
        if not expectedVersion:
            raise RuntimeError('The local Tika server jar filename has no version.')
        alreadyRunning = checkPortIsOpen(serverHost, port)

        if alreadyRunning:
            response = requests.get(serverEndpoint + '/version', timeout=60, verify=False)
            if response.status_code != 200 or response.text.strip() != 'Apache Tika ' + expectedVersion:
                raise RuntimeError('The Tika server already using port %s is not Apache Tika %s.' % (port, expectedVersion))
        else:
            status = startServer(tikaServerJar, TikaJava, TikaJavaArgs, serverHost, port, classpath, config_path)
            if not status:
...
def startServer(tikaServerJar, java_path = TikaJava, java_args = TikaJavaArgs, serverHost = ServerHost, port = Port, classpath=None, config_path=None):
    ...
    command = [java_path] + shlex.split(java_args) + [
        '-cp', classpath, 'org.apache.tika.server.core.TikaServerCli', '--port', str(port), '--host', host,
    ]
    if config_path:
    ...
    try:
        TikaServerProcess = Popen(command, stdout=logFile, stderr=STDOUT)
    ...
    if not is_started:
        ...
        TikaServerProcess.terminate()
        try:
            TikaServerProcess.wait(timeout=10)
        except TimeoutExpired:
            TikaServerProcess.kill()
            TikaServerProcess.wait(timeout=10)
        TikaServerProcess = None
        return False
    else:
        return True
