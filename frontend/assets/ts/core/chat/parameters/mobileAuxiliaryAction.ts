...
type ChatMobileAuxiliaryAction = 'none' | 'agent_mode' | 'agent_compact' | 'export' | 'configuration' | 'favorite' | 'tools' | 'token_counter' | 'voice' | 'voice_call' | 'camera' | 'attach' | 'prompts' | 'character_map' | 'new_conversation';

const CHAT_MOBILE_AUXILIARY_ACTION_NONE: ChatMobileAuxiliaryAction = 'none';

const CHAT_MOBILE_AUXILIARY_ACTIONS: ReadonlyArray<ChatMobileAuxiliaryAction> = Object.freeze(['none', 'agent_mode', 'agent_compact', 'export', 'configuration', 'favorite', 'tools', 'token_counter', 'voice', 'voice_call', 'camera', 'attach', 'prompts', 'character_map', 'new_conversation']);
