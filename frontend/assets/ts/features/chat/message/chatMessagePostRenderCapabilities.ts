...
const ATTACHMENT_THUMBNAIL_IMAGE_SELECTOR = 'img[data-chat-attachment-thumbnail="true"]';
...
const CHAT_IMAGE_LIFECYCLE_SELECTOR = `${INLINE_TOOL_IMAGE_SELECTOR}, ${ASSISTANT_ARTICLE_IMAGE_SELECTOR}, ${ATTACHMENT_THUMBNAIL_IMAGE_SELECTOR}`;
...
const classifyChatMessagePostRenderCapabilities = (markup: string): ReadonlySet<ChatPostRenderCapability> => {
    ...
    if (markup.includes(INLINE_TOOL_IMAGE_CLASS) || markup.includes(ASSISTANT_ARTICLE_IMAGE_CLASS) || markup.includes('data-chat-attachment-thumbnail')) capabilities.add('image-lifecycle');
