...
const renderKnowledgeAttachmentRow = (context: ChatUIManagerContext, knowledge: KnowledgeAttachmentSummary): TrustedHtml => {
    ...
    if (knowledgeAttachmentId) {
        ...
        const leadingMarkup = context.dependencies.getCachedIcon('file-database', { size: 16, strokeWidth: 1.5 });
        const spinner = presentation.showSpinner ? uiHtml`<span class="loading-spinner file-status-spinner" aria-hidden="true"></span>` : uiHtml``;
        const bodyMarkup = completedCount > 0 ? uiHtml`<button type="button" class="chat-attachment-summary-card-body" data-action="${uiAttr(CHAT_ACTIONS.OPEN_KNOWLEDGE_ATTACHMENT_PREVIEW)}" data-knowledge-attachment-id="${uiAttr(knowledgeAttachmentId)}" data-knowledge-title="${uiAttr(title)}" aria-label="${uiAttr(title)}" data-tooltip="${uiAttr(title)}"><span class="file-preview-leading-icon" aria-hidden="true">${leadingMarkup}</span><span class="file-info"><span class="file-name">${uiText(title)}</span><span class="file-status">${spinner}${uiText(statusText)}</span></span></button>` : uiHtml`<div class="chat-attachment-summary-card-body"><span class="file-preview-leading-icon" aria-hidden="true">${leadingMarkup}</span><span class="file-info"><span class="file-name">${uiText(title)}</span><span class="file-status">${spinner}${uiText(statusText)}</span></span></div>`;
    ...
    return buildChatFilePreviewItemDocumentMarkup({
        leadingVisualMarkup: uiHtml`<span class="file-preview-leading-icon" aria-hidden="true">${context.dependencies.getCachedIcon('file-database', { size: 16, strokeWidth: 1.5 })}</span>`,
