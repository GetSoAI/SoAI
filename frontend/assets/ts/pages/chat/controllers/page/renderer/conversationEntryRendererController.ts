...
const renderVariantSlide = (host: ChatCurrentConversationRenderDependencies, variant: ComparisonTurnVariantEntry, conversationId: string): TrustedHtml => {
    ...
    const markup = host.conversationRuntime.requireMessages().renderMessage({ message: variant.message, conversationId, index: variant.messageIndex, comparisonTurn: variant.comparisonTurn, presentation: variant.presentation });
...
export const renderConversationEntryMarkup = (host: ChatCurrentConversationRenderDependencies, entry: ConversationRenderEntry, conversationId: string): TrustedHtml => {
    if (entry.type === 'message') {
        return host.conversationRuntime.requireMessages().renderMessage({ message: entry.message, conversationId, index: entry.index, comparisonTurn: entry.comparisonTurn, presentation: entry.presentation });
    }

    const slides = entry.variants.map((variant) => renderVariantSlide(host, variant, conversationId));
