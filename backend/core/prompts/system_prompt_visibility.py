...
def _get_soai_system_prompt_no_tools_replacements() -> Mapping[str, str | None]:
    return {
        ...
        "If SOAI_LOCAL_DATETIME appears unfilled (literal placeholder text) and a time tool is available, use it. If local weather is requested without a place, use a geographic IANA timezone city such as Europe/Rome -> Rome as the weather tool location hint, setting forecast_days to the requested window (1-14, default 3) and hourly_hours (1-48) only when intra-day detail is asked for. Empty user/assistant display names simply mean no name was configured; treat them as absent (not as errors).": "If SOAI_LOCAL_DATETIME appears unfilled (literal placeholder text), acknowledge that the current time was not provided by SoAI. Empty user/assistant display names simply mean no name was configured; treat them as absent (not as errors).",
