...
def _recover_running_input(
    ...
    *,
    ...
    assistants = sqlite_conn.execute(
        """
        SELECT id, request_id, finalized_at_ms, finish_reason, model_variant_index
        FROM webui_messages
    ...
    if not assistants:
        ...
        return
    request_ids = {assistant[1] for assistant in assistants}
    source_message_id: int | None = None
    if len(request_ids) != len(assistants) or len(assistants) > expected_variant_count:
        terminal_state, terminal_code = ("effect_unknown", "interrupted_effect_unknown")
    elif any(assistant[2] is None for assistant in assistants):
        terminal_state, terminal_code = ("effect_unknown", "interrupted_effect_unknown")
    else:
        failed_assistant = next(
            (assistant for assistant in assistants if assistant[3] in {"cancelled", "error"}),
            None,
        )
        if failed_assistant is not None:
            terminal_state, terminal_code = _terminal_state_for_finish_reason(
                str(failed_assistant[3]),
            )
            source_message_id = int(failed_assistant[0])
            ...
            final_assistant = next(
                (
                    ...
                    if assistant[4] == expected_variant_count - 1
                ),
                None,
            )
            if final_assistant is None:
                raise StateError("Recovered final assistant variant is unavailable.")
            source_message_id = int(final_assistant[0])
        else:
            sqlite_conn.execute(
                """
                UPDATE webui_conversation_inputs
