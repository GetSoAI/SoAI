"""SoAI - Exact V1 database schema validation [backend/database/v1_schema_validation.py]"""
...
from database.webui_identity_schema_validation import validate_current_v1_webui_identity_rows

__all__ = ("SchemaObjectKey", "validate_current_v1_database_schema")
...
def _expected_schema_objects(task_catalog: TaskTypeCatalog) -> dict[SchemaObjectKey, str]:
    ...
    try:
        ...
        apply_current_v1_schema_objects(expected, task_catalog)
...
def _raise_schema_mismatch(
    *,
    ...
    raise StateError(
        "Current V1 database schema does not match the canonical shape.",
        details={
...
def validate_current_v1_database_schema(
    ...
    validate_current_v1_webui_identity_rows(conn)
