...
class GpuCapabilitiesService:
    ...
    async def get_capabilities(self, enrich_fn: Callable[[JSONDict], JSONDict]) -> JSONDict:
        ...
        if (
            ...
            and cache_revision == gpu_info_revision
        ):
            return cached
        ...
        async def compute() -> JSONDict:
            ...
            try:
                ...
                if cached is not None and _capabilities_have_gpus(cached):
                    return cached
                ...
                if cached is not None and _capabilities_have_gpus(cached):
                    return cached
                return build_probe_error_payload("gpu_error", str(exception))
            current_revision = self._gpu_info_cache_service.revision()
            payload_has_gpus = _capabilities_have_gpus(payload)
            if current_revision != probe_revision and not payload_has_gpus:
                if cached is not None and _capabilities_have_gpus(cached):
                    return cached
                return build_probe_error_payload(
                    "stale_probe",
    ...
    async def get_cached_capabilities(self) -> JSONDict | None:
        gpu_info_revision = self._gpu_info_cache_service.revision()
        async with self._cache_lock:
            if self._cache is None or self._cache_gpu_info_revision != gpu_info_revision:
                return None
            return copy.deepcopy(self._cache)


def _capabilities_have_gpus(payload: JSONDict) -> bool:
    gpus_value = payload.get("gpus")
    return isinstance(gpus_value, dict) and any(
        isinstance(entry, dict) for entry in gpus_value.values()
    )
