...
def sync_list_gpu_slots(
    *,
    ...
    deps = build_slot_payload_dependencies(
        executor=executor,
        storage=storage,
    ...
    with storage.lock:
        capabilities, inventory, payload = get_capabilities_and_inventory(deps, allow_create=True)
        devices_payload, version = (
            devices(payload),
            payload.get("version", GPU_SLOT_FILE_VERSION),
        )
        if device_id:
            ...
            cap_entry = get_capabilities_for_device(
                capabilities,
                device_id,
                normalize_gpu_index(entry_copy.get("gpu_index")),
            )
